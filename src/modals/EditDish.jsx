...
export default function EditDish({ navigation, route }) {
    ...
    const { dish } = route.params ? route.params : {};
    const [categories, setcategories] = useState();
    const [id, setid] = useState(dish.id);
    const [name, setname] = useState(dish.name);
    const [photo, setphoto] = useState(dish.photo);
    const [sizes, setsizes] = useState(dish.sizes);
    const [details, setdetails] = useState(dish.details);
    ...
    const rendersizes = sizes?.map((item, index) => {
        return (
            <Size Colors={Colors} Title={item.name} Price={item.price} onPress={() => {
                navigation.navigate('Editsize', { size: item, index, sizes, setsizes, send: true });
            }} />
        );
    });
    const GetCategories = () => {
    ...
    const GetDish = () => {
        ...
        axios
            ...
            .then((response) => {
                ...
                setphoto(response.data.dish.photo);
                setid(response.data.dish.id);
    ...
    const sendForm = () => {
        ...
        axios
            .post(`${BaseUrl}show/dishUpdate`, data, config)
            .then((response) => {
                ...
                <View style={[{ height: "50%", width: '100%', overflow: 'hidden', backgroundColor: Colors.fourth, borderTopEndRadius: 50, borderTopStartRadius: 50 }]}>
                    ...
                    <ScrollView style={[barakocta.p3, barakocta.pt4]}>
                        ...
                        <TouchableOpacity
                            ...
                            onPress={() => {
                                navigation.navigate('Addsize', { setsizes, sizes, id, send: true });
                            }}
                        >
