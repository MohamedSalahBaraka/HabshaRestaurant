...
export default function Editsize({ navigation, route }) {

    const { setsizes, sizes, index, size, send } = route.params ? route.params : {};
    ...
    const sendForm = () => {
        ...
        axios
            .post(`${BaseUrl}show/sizeupdate`, bodyParameters, config)
            .then((response) => {
                navigation.goBack();
                setIsLoading(false);
            })
            ...
            <View style={{ flex: 1, flexDirection: 'column', justifyContent: 'flex-end', backgroundColor: 'transparent', }}>
                ...
                <View style={[{ height: "50%", width: '100%', overflow: 'hidden', backgroundColor: Colors.fourth, borderTopEndRadius: 50, borderTopStartRadius: 50 }]}>
                    ...
                    <ScrollView style={[barakocta.p3, barakocta.pt4]}>
                        <Text style={[barakocta.tRight, barakocta.mb3, { color: Colors.tertiary, fontFamily: 'Cairo', fontSize: 20 },]}>تعديل حجم</Text>
                        ...
                        >
                            <Text style={[barakocta.tCenter, { color: Colors.fourth, fontFamily: 'Cairo', fontSize: 16 },]}>عدل</Text>
                        </TouchableOpacity>
